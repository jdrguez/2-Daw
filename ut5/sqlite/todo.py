...
class Task:
    ...
    def update(self):
        ...
        self.cur.execute(self.cur(sql, dict(name=self.name, done=self.done, id=self.id)))
        self.con.commit()

    def check(self):
        """Marca la tarea como completada. Haz uso también de .update()"""
        sql = 'UPDATE tasks SET done = :done where id = :id'
        self.cur.execute(self.cur(sql, dict(done=TASK_DONE_SYMBOL)))
        self.con.commit()

    def uncheck(self):
        """Marca la tarea como no completada. Haz uso también de .update()"""
        sql = 'UPDATE tasks SET done = :done where id = :id'
        self.cur.execute(self.cur(sql, dict(done=TASK_PENDING_SYMBOL)))
        self.con.commit()
