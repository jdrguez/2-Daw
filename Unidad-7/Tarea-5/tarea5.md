...
- Suponga que estamos trabajando con la base de datos jardineria y queremos saber optimizar las siguientes consultas. ¿Cuál de las dos sería más eficiente?. Se recomienda hacer uso de EXPLAIN para obtener información sobre cómo se están realizando las consultas.
  ...
  ```
  > Respuesta:
    Claramente la primera consulta, puesto que, además de ser más explicativa y clara, no tendra que recorrer todas la fechas que esten en la base de datos
    se centrará únicamente en el año. El problema reside que tendrá que analizar todas las consultas para ver si pertenecen a dicho año. La segunda consulta
    va más allá, esta se encarga de analizar dia y mes de todas las fechas en un año.
...
- Optimiza la siguiente consulta creando índices cuando sea necesario. Se recomienda hacer uso de EXPLAIN para obtener información sobre cómo se están realizando las consultas.
  ...
  AND cliente.nombre_cliente LIKE 'A%';
  -- Sin modificar:
  EXPLAIN   SELECT *
    ->   FROM cliente INNER JOIN pedido
    ->   ON cliente.codigo_cliente = pedido.codigo_cliente
