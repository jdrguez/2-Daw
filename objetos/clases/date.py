...
class Date:
    ...
    def weekday(self) -> int:
        ...
        zeller_form = day + ((13 * (month + 1 )) // 5 ) + century_year + (century_year // 4) + (century // 4) - (2 * century)
    ...
    def __sub__(self, other: Date | int) -> int | Date:
        ...
        if isinstance(other, Date):
            total_days_without_date_self = self.get_total_days() + self.get_total_days(leap_year= True)
            total_days_without_date_other = other.get_total_days() + other.get_total_days(leap_year= True)
            actual_self_days = sum([self.days_in_month(month, self.year) for month in range(self.month)]) + self.day
            actual_other_days = sum([other.days_in_month(month, other.year) for month in range(other.month)]) + other.day
