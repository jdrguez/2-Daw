...
trap "echo Se procede a interrumpir la ejecución..; exit 0"
INT
...
do
    ...
    piofidCPUprocesos=$(ps axo pid= --sort -pcpu | head -n 1)

    procesosMEMORIA=$(ps axo pmem= --sort -pnem | head -n 1)
    ...
    then
        echo "Los procesos obtenidos superen los limites establecidos en un inicio, porfavor determina que proceso debemos tratar: "
        read -p "Seleccione entre los siguientes = CPU o Memoria" opcion
        if [[ "$opcion" == "CPU" ]]
        then
            echo "Has seleccido  la opción de verificar la carga en CPU."
            ps p $piofidCPUprocesos
            break
        fi
        
        if [[ "$opcion" == "Memoria" ]]
        then
            echo "Has seleccionado la opción de verificar la carga en Memoria."
...
case "$choice" in
    ...
    ;;
    b|B|2) echo "Disminuir en 3 puntos la prioridad del proceso"
    aux=$(( aux + 3 ))
    renice $aux $1
    ;;
